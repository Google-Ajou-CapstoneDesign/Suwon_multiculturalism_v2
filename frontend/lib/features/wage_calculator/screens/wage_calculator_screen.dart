...
/// 임금·체불 정밀 진단 계산기. 필드 내용은 프론트엔드_계산기.html, 단계별 위저드
/// 구성은 프론트엔드_최종.html의 "정밀 임금계산기" 뷰를 참고했다 — 3단계
/// (① 급여 계약 방식~사업장 규모 ② 근무 조건 입력 ③ 세금 및 공제)를 거쳐 결과를
/// 보여준다. 계산 결과는 참고용 추정치이며, 진정서 금액은 근로감독관 조사에서
/// 확정된다(WageNavigatorScreen 4단계 fillcard와 동일한 원칙).
...
class _WageCalculatorScreenState extends State<WageCalculatorScreen> {
  ...
  static const _stepTitles = ['급여 계약 방식', '근무 조건 입력', '세금 및 공제'];

  late final _amountCtrl = TextEditingController(
    text: _amountDefault(_contractType).toStringAsFixed(0),
  );
  ...
  void _fillFromWorklogDemo() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('데모입니다 — 실제 근무기록장 연동은 추후 지원됩니다. 샘플 값을 채워 넣었어요.'),
      ),
    );
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const Text(
                  '출처: 국민연금공단·국민건강보험공단·고용보험 2026년 요율 고시',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _StepHeader(
            title: _showResult ? '진단 결과' : _stepTitles[_step],
            stepIndex: _step,
          ...
          _StepFooter(
            resultMode: _showResult,
            showPrev: _step > 0,
            nextLabel: _step == _stepTitles.length - 1 ? '✨ 정밀 진단 결과 보기' : '다음',
            onPrev: _goPrev,
            onNext: _goNext,
          ),
        ],
              ...
              Row(
                children: [
                  Expanded(
                    child: _ChoiceButton(
                      label: '시급',
                      selected: _contractType == ContractType.hourly,
                      color: AppColors.primary,
                      onTap: () => _setContractType(ContractType.hourly),
                    ),
                  ),
                  ...
                  Expanded(
                    child: _ChoiceButton(
                      label: '일급',
                      selected: _contractType == ContractType.daily,
                      color: AppColors.primary,
                      onTap: () => _setContractType(ContractType.daily),
                    ),
                  ),
                  const SizedBox(width: 6),
                  Expanded(
                    child: _ChoiceButton(
                      label: '주급',
                      selected: _contractType == ContractType.weekly,
                      color: AppColors.primary,
                      onTap: () => _setContractType(ContractType.weekly),
                    ),
                  ),
                ],
              ),
              ...
              Row(
                children: [
                  Expanded(
                    child: _ChoiceButton(
                      label: '월급',
                      selected: _contractType == ContractType.monthly,
                      color: AppColors.primary,
                      onTap: () => _setContractType(ContractType.monthly),
                    ),
                  ),
                  const SizedBox(width: 6),
                  Expanded(
                    child: _ChoiceButton(
                      label: '연봉',
                      selected: _contractType == ContractType.annual,
                      color: AppColors.primary,
                      onTap: () => _setContractType(ContractType.annual),
                    ),
                  ),
                ],
              ),
          ...
          child: Column(
            ...
            children: [
              ...
              _ChoiceButton(
                ...
                color: AppColors.secondary,
                onTap: () =>
                    setState(() => _checkPeriod = CheckPeriod.thisPeriod),
              ),
              const SizedBox(height: 7),
              _ChoiceButton(
                ...
                color: AppColors.secondary,
                onTap: () =>
                    setState(() => _checkPeriod = CheckPeriod.unpaidLong),
              ),
                ...
                Row(
                  children: [
                    Expanded(
                      child: _ChoiceButton(
                        label: '1주일치만',
                        selected: _monthScope == MonthScope.week,
                        color: AppColors.primary,
                        small: true,
                        onTap: () => _setMonthScope(MonthScope.week),
                      ),
                    ),
                    const SizedBox(width: 6),
                    Expanded(
                      child: _ChoiceButton(
                        label: '한 달 전체',
                        selected: _monthScope == MonthScope.month,
                        color: AppColors.primary,
                        small: true,
                        onTap: () => _setMonthScope(MonthScope.month),
                      ),
                    ),
                  ],
                ),
  ...
  Widget _buildStep2() {
    final showBelievedHint =
        _checkPeriod == CheckPeriod.unpaidLong &&
        (_contractType == ContractType.monthly ||
            _contractType == ContractType.annual);
    final monthlyAmtForHint = _contractType == ContractType.annual
        ? _num(_amountCtrl) / 12
        : _num(_amountCtrl);
    ...
    return Column(
      ...
      children: [
        Container(
          padding: const EdgeInsets.all(15),
          decoration: BoxDecoration(
            color: AppColors.blueBg.withValues(alpha: 0.35),
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              _NumberField(
                label: _amountLabel(_contractType),
              ...
              if (_contractType == ContractType.daily) ...[
                _NumberField(
                  ...
                  help: '하루에 보통 몇 시간 일하시나요? 주휴수당·통상시급 계산의 기준이 돼요',
                  onChanged: () => setState(() {}),
                ),
                const SizedBox(height: 9),
                _NumberField(
                ...
                const SizedBox(height: 9),
                _NumberField(
                  label: _monthScope == MonthScope.week
                      ? '이번 주 근무 일수'
                      : '이번 달 근무 일수',
              ...
              _NumberField(
                label: '연장근로 (시간)',
                controller: _otCtrl,
                help: '하루 8h·주 40h를 넘긴 시간',
                onChanged: () => setState(() {}),
              ),
              const SizedBox(height: 9),
              _NumberField(
                label: '야간근로 22~06시 (시간)',
                controller: _ntCtrl,
                onChanged: () => setState(() {}),
              ),
              const SizedBox(height: 9),
              ...
              if (_checkPeriod == CheckPeriod.unpaidLong) ...[
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _NumberField(
                        ...
                        help: '위 근무조건이 매달 같다고 가정한 근사치예요',
                        onChanged: () => setState(() {}),
                      ),
                      const SizedBox(height: 9),
                      _NumberField(
                        label: '그 기간 동안 총 받기로 약속된 금액 (전체 합계)',
                        controller: _believedCtrl,
                        help: '한 달치가 아니라 전체 기간 합계예요. 예: 월급×개월수',
                        onChanged: () => setState(() {}),
                      ),
                      const SizedBox(height: 9),
                      _NumberField(
                        label: '그 기간 실제로 받은 금액 합계 (못 받았으면 0)',
                        controller: _receivedTotalCtrl,
                        onChanged: () => setState(() {}),
                      ),
                      if (showBelievedHint) ...[
                        const SizedBox(height: 6),
                ...
                _NumberField(
                  label: '실제로 통장에 들어온 금액 (원)',
                  controller: _receivedCtrl,
                  help: '나눠서 받았으면 모두 더해서 적어주세요',
                  onChanged: () => setState(() {}),
                ),
            ],
          ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 9),
              _TaxOptionCard(
                value: TaxType.socialInsurance,
                groupValue: _taxType,
                title: '4대보험 적용 (약 9.7%)',
                subtitle: '국민연금·건강보험·고용보험이 정식으로 빠지는 월급',
                onChanged: (v) => setState(() => _taxType = v),
              ),
              const SizedBox(height: 7),
              _TaxOptionCard(
                value: TaxType.businessTax,
                groupValue: _taxType,
                title: "3.3%만 떼요 (사업소득세)",
                subtitle: "⚠️ 회사가 '프리랜서'로 신고했을 가능성",
                onChanged: (v) => setState(() => _taxType = v),
              ),
              const SizedBox(height: 7),
              _TaxOptionCard(
                ...
                groupValue: _taxType,
                title: '세금을 안 떼요',
                subtitle: '현금이나 전액 그대로 수령',
                onChanged: (v) => setState(() => _taxType = v),
              ),
              const SizedBox(height: 7),
              _TaxOptionCard(
                value: TaxType.unknown,
                groupValue: _taxType,
                title: '잘 모르겠어요',
                subtitle: '두 가지 경우를 함께 보여드려요',
                onChanged: (v) => setState(() => _taxType = v),
              ),
            ...
            children: [
              ...
              if (_roomOn) ...[
                ...
                _Dropdown<RoomType>(
                  ...
                  items: const {
                    RoomType.dorm: '기숙사',
                    RoomType.studio: '원룸·주택',
                    RoomType.mealOnly: '식사만',
                  },
          ...
          Row(
            children: [
              ...
              Container(
                ...
                child: Text(
                  resultMode ? '완료' : '${stepIndex + 1} / $totalSteps',
                  style: const TextStyle(
                    fontSize: 10,
                    fontWeight: FontWeight.w700,
                    color: AppColors.primary,
